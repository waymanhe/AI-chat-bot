...
from qwen_agent_local.llm.schema import ASSISTANT, DEFAULT_SYSTEM_MESSAGE, FUNCTION, SYSTEM, USER, Message
from qwen_agent_local.log import logger
from qwen_agent_local.settings import DEFAULT_MAX_INPUT_TOKENS
from qwen_agent_local.utils.tokenization_qwen import tokenizer
from qwen_agent_local.utils.utils import (extract_text_from_message, format_as_multimodal_message, format_as_text_message,
...
class BaseChatModel(ABC):
    ...
    def quick_chat_oai(self, messages: List[dict], tools: Optional[list] = None) -> dict:
        ...
        def _convert_to_qwen_agent_local_messages(messages):
            new_messages = []
            for msg in messages:
        ...
        for rsp in self.chat(
                messages=_convert_to_qwen_agent_local_messages(messages),
                functions=functions,
                stream=True,
        ):
