...
def save_docx_as_json(docx_path, output_dir):
    doc = docx.Document(docx_path)
    # 将每个段落作为一页，结构与 minerU PDF 解析结果兼容
    pages = []
    ...
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(data, f, ensure_ascii=False, indent=4)
    print(f" -> DOCX 已保存为 JSON: {json_path}")

def parse_pdf_documents_requests(pdf_directory: str = PDF_REPORTS_DIR):
    pdf_files = [f for f in os.listdir(pdf_directory) if f.lower().endswith(".pdf")]
    docx_files = [f for f in os.listdir(pdf_directory) if f.lower().endswith(".docx")]
    if not pdf_files and not docx_files:
        print(f"在目录 '{pdf_directory}' 中没有找到PDF或DOCX文件。")
        return
    # 先处理 docx 文件
    for docx_file in docx_files:
        docx_path = os.path.join(pdf_directory, docx_file)
        save_docx_as_json(docx_path, PROCESSED_REPORTS_DIR)
    # 再处理 PDF 文件（原有 minerU 流程）
    if pdf_files:
...
if __name__ == '__main__':
    if not any(f.lower().endswith((".pdf", ".docx")) for f in os.listdir(PDF_REPORTS_DIR)):
        print(f"'{PDF_REPORTS_DIR}' 目录为空，请先放置PDF或DOCX文件。")
    else:
